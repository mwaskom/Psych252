...
$$Sum of Squares: SS = \sum\limits_{i=1}^n (x_i - \bar{x})^2 = 
\sum\limits_{i=1}^n (x_i)^2 - \frac{(\sum\limits_{i=1}^n x_i)^2}{n}, df = n-1$$
$$Sum of Products: SP = \sum\limits_{i=1}^n (x_i - \bar{x})(y_i - \bar{y})$$
$$Covariance:cov(x,y) = \frac{SP}{n-1}$$
$$Correlation (\rho): r(x,y) = \frac{SP}{\sqrt{SS_x * SS_y}} = \frac{cov(x,y)}{\sigma_x \sigma_y}$$
$$Linear function: \hat{Y} = a * bX, a=intercept, b=slope$$
$$Slope of regression: b = \frac{SP}{SS_x} =  \frac{cov(x,y)}{s^2_x}$$
$$Intercept of regression: a = \hat{y} - b * \hat{x}$$
$$Statistic of regression: t = r * \sqrt{\frac{n-2}{1-r^2}}$$
$$Standard error of prediction: stErr_\hat{y} = \sqrt{\frac{SS_y}{(n-2)}*(1-r^2)}$$
