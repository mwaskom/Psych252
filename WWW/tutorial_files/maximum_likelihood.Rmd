...
toss the coin $n = 100$ times and observe $r = 45$ Heads. A familiar estimate of $p$ is: $$\hat{p}=\frac{r}{n}=\frac{45}{100}=0.45$$

In what sense is this estimate, $\hat{p}=\frac{r}{n}$, the *“best”* estimate of $p$? One answer that is already familiar to us is that $\hat{p}$ is an **unbiased estimate** of $p$; i.e., this choice of estimate leads to a perfect fit between the expected ($\frac{100}{2}$ = 50 Heads) and the observed number of Heads (45 Heads). We now derive another answer by applying the principle of **maximum likelihood**.

The probability (or **likelihood**, $l$) of observing the *particular* sequence of $r$ Heads and $n-r$ Tails that we observed is: $$l(p)={p}^{r}{(1-p)}^{n-r}$$
...
has the chi-square distribution under the null that $p = p0$. As it turns out, $G^2$ tends to have approximately the same value as the familiar, Pearson chi-square index,
