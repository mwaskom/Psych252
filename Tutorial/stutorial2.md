...
Now, we might want to treat some variables as qualitative, nominal **factors** rather than continuous, numeric **integers**. In R, we must specify which variables to treat as factors if the **levels** (unique values) of the variable are composed of numbers instead of strings. 
...
We can also plot age as a function of age category:
...
Next, let's view a scatter plot of optimism by age. We'll also add in a line to get a sense of the general trend of the data; here, we'll plot a non-parametric best-filling curve in red using the ["lowess"](http://rss.acs.unt.edu/Rdoc/library/gplots/html/lowess.html) function.
...
To examine the best-fitting linear relationship between quantiative variables (e.g., X and Y), we can use the function **lm()**. For instance, we might want to investigate the relationship between age and optimism, depicted in the plot above. As age increases, how does optimism change?


```r
rs2 = lm(optmism ~ age, data = d2)
print(summary(rs2))
```
...
We can also plot the regression line, fit by least squares regression, on top of the scatterplot. While we're doing this, let's also change the names of the x- and y-axis labels:

```r
plot(d2$age, d2$optmism, main = "Optimism vs Age", xlab = "Age", ylab = "Optimism")
abline(rs2, col = "red")
```
...
R also allows us to read data that's stored in online repositoies. We can provide the website URL, and R will read in the table:
...
str(rtw.table)  # description of dataframe
```
...
Here, the output suggests that there is no significant relationship between unemployment rate and cost of living.
...
### Removing outliers
These last diagnostic plots suggest that observations 6 and 34 are outliers. What would the plot look like if these outliers were removed?  
...
Now, one might ask if the right to work law affects cost of living, when controlling for other variables. This is an example of multivariate analysis. To examine this question, we can enter multiple predictors into the lm() function:
